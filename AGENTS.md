...
- Use `./bin/test` for full verification. It rebuilds the app, builds and runs all five QtTest suites with `QT_QPA_PLATFORM=offscreen`, then runs regular in-memory and seeded automation startup smoke checks.
...
- Test ownership: `appcontroller_tests` covers playback state, watch tracking, and QSettings-backed playback preferences; `repository_tests` covers SQLite, migrations, and models; `youtube_tests` covers response parsing and refresh orchestration through a fake client with no network; `theme_tests` covers palette parsing, built-in resources, and selection; `navigation_tests` covers full and Simple UI routes in deterministic automation mode.
- Before any UI navigation or testing, read `AUTOMATION.md` for the safe launch mode, fixture IDs, selectors, Loader waits, and route recipes; do not invent selectors or launch against user data.

## Architecture

- `yt-client.pro` defines the application. `ytclient_core.pri` owns repository, models, YouTube client, and refresh service sources shared by the app and four test projects.
