...
- Test ownership: `appcontroller_tests` covers playback state, watch tracking, and QSettings-backed playback preferences; `repository_tests` covers SQLite, migrations, and models; `youtube_tests` covers response parsing and refresh orchestration through a fake client with no network; `theme_tests` covers palette parsing, built-in resources, and selection; `navigation_tests` covers full and Simple UI routes in deterministic automation mode; `automation_tests` (`tests/automation_test.cpp`, `tests/automation_tests.pro`) covers the JSON sequence parser, runner, and actual CLI behavior.
- Before any UI navigation or testing, read `AUTOMATION.md` for the safe launch modes, JSON sequence schema, fixture IDs, selectors, timing rules, and runnable examples; do not invent selectors or launch against user data. Use `./build/yt-client --automation-sequence /tmp/run.json [--automation-ui full|simple]` for scripted navigation; the runner uses the disposable seeded fixture, fake player, and app-only screenshots.

## Architecture

- `yt-client.pro` defines the application. `ytclient_core.pri` owns repository, models, YouTube client, and refresh service sources shared by the app and test projects.
- `src/main.cpp` creates the sole `AppController`, registers it as QML singleton `YtClient.App`, and loads `qrc:/qml/Main.qml` or `qrc:/qml/SimpleMain.qml` from the saved UI setting.
